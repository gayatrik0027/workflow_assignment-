...
An employee submits a leave request through the company's HR system. The system checks the employee's available leave balance before sending the request to the manager.

### BPMN Process Flow

1. Employee submits a leave request.
2. The system checks the employee's leave balance.
3. An exclusive gateway checks whether sufficient leave balance is available.
4. If sufficient leave balance is available, the request is sent to the manager.
...
A customer places an online order. The system checks product availability and processes the payment before preparing and shipping the order.
...
The three BPMN models demonstrate how basic BPMN building blocks can be used to represent business processes, decision points, alternative process paths, notifications, and process completion using Camunda Modeler.
